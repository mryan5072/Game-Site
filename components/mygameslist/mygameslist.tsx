...
const MyGamesList: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchReviewedGames = async () => {
      setLoading(true);
      try {
        const ratingsRef = collection(
          firestore,
          `individualGameRatings/${userUID}/ratings`
        );
        const snapshot = await getDocs(ratingsRef);

        if (!snapshot.empty) {
          const gameIds = snapshot.docs.map((doc) => Number(doc.id));
          setReviewedGameIds(gameIds);
          console.log(gameIds);
        } else {
          setReviewedGameIds([]);
        }
      } catch (err: any) {
        setError("Failed to fetch reviewed games.");
      } finally {
        setLoading(false);
      }
    };
    ...
    try {
      const response = await fetch(
        `/api/getGames?page=${page}&search=${encodeURIComponent(
          searchQuery
        )}&limit=${limit}&platform=${platform}&category=${category}&genre=${genre}&sortby=${sortBy}&gameIds=${reviewedGameIds.join(
          ","
        )}`,
        {
          method: "POST",
  ...
  useEffect(() => {
    window.scroll(0, 0);
    if (userUID && reviewedGameIds.length > 0) {
      fetchGames();
    }
  }, [
    userUID,
        ...
        <DialogContent>
          ...
          <FormControl fullWidth margin="normal">
            ...
            >
              ...
              <MenuItem value="turn-based-stategy-tbs">
                Turn Based Strategy (TBS)
              </MenuItem>
              <MenuItem value="hack-and-slash-beat-em-up">
